...
#define EXPECT_EQ_DOUBLE(expect, actual) EXPECT_EQ_BASE((expect) == (actual), expect, actual, "%.17g") // TODO: why? %.17g
...
#define TEST_LITERAL(target, json)\
    do {\
        lept_value v;\
        v.type = LEPT_FALSE;\
        EXPECT_EQ_INT(LEPT_PARSE_OK, lept_parse(&v, json));\
...
static void test_access_boolean() {
	...
	lept_value v;
	lept_init(&v);
	lept_set_number(&v, 1.3);
	EXPECT_EQ_DOUBLE(1.3, lept_get_number(&v));
	lept_free(&v);
}
...
int main() {
	...
	while ((c = getc(file)) != EOF) str[cur++] = c;	
	fclose(file);
	str[cur] = '\0';
	lept_value tmp;

	printf("Now parse...\n");
	int ret = lept_parse(&tmp, str);	
	if (ret != LEPT_PARSE_OK) printf("Error code %d.\n", ret);
	else printf("OK\n");
	
	printf("Now stringfy...\n");
	int len = -1;
	char* outputstr;
	ret = lept_stringify(&tmp, &outputstr, &len);
	if (ret != LEPT_STRINGIFY_OK) printf("Error code %d.\n", ret);
	else printf("OK\n");
